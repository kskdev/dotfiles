...
- Linux環境用の Vim ソースビルドスクリプトも追加

## ファイル構成

```
Vim/
├── install.sh         # vimrc インストールスクリプト
├── build.sh           # Vimビルドスクリプト (Linux)
│
├── vimrc              # エントリポイント
...
# Vim ソースビルド (For Linux environment)

ddc.vim 等のプラグインは特定のバージョン以上を要求するため注意
...
- GUI なし (CUI) の Vim を `/opt/vim91` にインストール
- インストール完了後，`.zshrc` または `.bashrc` へ PATH と alias を自動追記

### オプション

| 環境変数     | 説明                  | デフォルト   |
| ------------ | --------------------- | ------------ |
| `VIM_PREFIX` | インストール先        | `/opt/vim91` |
